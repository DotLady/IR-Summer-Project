...
# -------------------------------------- Main Control Loop --------------------------------------
    ...
    while (sim.simxGetConnectionId(clientID) != -1):
        ...
        groundrobotposition = sim.simxGetObjectPosition(
            clientID, body, floor, sim.simx_opmode_blocking)
        groundrobotangle_Euler = sim.simxGetObjectOrientation(
            clientID, body, floor, sim.simx_opmode_blocking)[1][2]
        ...
        if (len(pathlist) >= 1):
            ...
            if (fun_gnd.checkangle(groundrobotangle, pathlist[0][2]) == True):
                ...
                groundrobotangle_Euler = sim.simxGetObjectOrientation(
                    clientID, body, floor, sim.simx_opmode_blocking)[1][2]
                groundrobotangle = fun_gnd.changeangletoeurrle(
                ...
                sim.simxSetJointTargetVelocity(
                    clientID, leftMotor, 5.0 - rotatespeed, sim.simx_opmode_oneshot)
                sim.simxSetJointTargetVelocity(
                    clientID, rightMotor, 5.0 + rotatespeed, sim.simx_opmode_oneshot)
                groundrobotposition = sim.simxGetObjectPosition(
                    clientID, body, floor, sim.simx_opmode_blocking)
                groundrobotangle_Euler = sim.simxGetObjectOrientation(
                    clientID, body, floor, sim.simx_opmode_blocking)[1][2]
